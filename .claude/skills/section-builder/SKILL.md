...
Токени дизайну (кольори, шрифти) забирай у `src/styles/partials/_vars.scss` / `_fonts.scss` — там зараз плейсхолдери, які заповнюються з макета один раз, а не по секції.
